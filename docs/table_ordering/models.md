...
# SQLAlchemy Models for Multi‑Restaurant QR Ordering  
*(int primary keys + public_id for frontend use)*

```python
from datetime import datetime, time, date
from sqlalchemy import (
    Column, Integer, String, Boolean, DateTime, Date, Time,
    Text, Enum, ForeignKey, SmallInteger, JSON, UniqueConstraint, Index
)
...
class Restaurant(Base):
    ...
    public_id   = Column(String(36), unique=True, nullable=False)     # UUID/slug for frontend
    slug        = Column(String, unique=True, nullable=False)
    name        = Column(String, nullable=False)
    tz          = Column(String, nullable=False, default="UTC")       # IANA timezone
...
class Session(Base):
    ...
    __table_args__ = (
        # ensure one active session per table per restaurant
        Index(
...
class Member(Base):
    ...
    device_id   = Column(String, nullable=False)          # random UUID from frontend
...
class MenuItem(Base):
    ...
    kind            = Column(String)                 # 'food', 'beverage', etc.
